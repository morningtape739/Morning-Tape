...
(function () {
  ...
  async function fetchEquity(symbol, days) {
    ...
    const url = `https://stooq.com/q/d/l/?s=${ASSETS[symbol].stooq}&i=d`;
    let csv;
    try {
      const r = await fetch(url);
      if (!r.ok) throw new Error('direct fail');
      csv = await r.text();
      if (!csv || csv.length < 100) throw new Error('empty');
    } catch (e) {
      const r2 = await fetch(`https://corsproxy.io/?${encodeURIComponent(url)}`);
      if (!r2.ok) throw new Error(`Stooq failed for ${symbol}`);
      csv = await r2.text();
    }
    const lines = csv.trim().split('\n');
    if (lines.length < 2) throw new Error(`No data ${symbol}`);
    const head = lines[0].toLowerCase().split(',');
    const di = head.indexOf('date'), ci = head.indexOf('close');
    if (di < 0 || ci < 0) throw new Error(`Bad CSV ${symbol}`);
